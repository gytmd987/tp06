...
int Deck::put_card(vector<int> put_list, int mp) {
    sort(put_list.begin(), put_list.end(), greater<>());
    int a = mp;
    for (unsigned int i = 0; i < put_list.size(); i++) {
        Card tmp = card_hand[put_list[i]];
        a -= tmp.get_mp();

    }
    if (a < 0) {
        printf("사용가능한 mp를 넘었습니다. 다시 입력해주세요.\n");
        return a;
    ...
    for (unsigned int i = 0; i < put_list.size(); i++) {
        Card tmp = card_hand[put_list[i]];
        if (card_hand.size() > 1)
            card_hand.erase(card_hand.begin() + put_list[i]);
        else
            card_hand.clear();
...
void Deck::use_magic_card(vector<int> put_list, Player& Px, Player& Py) {
    sort(put_list.begin(), put_list.end(), greater<>());
    for (unsigned int i = 0; i < put_list.size(); i++) {
        Card tmp = card_hand[put_list[i]];
        if (card_hand.size() > 1)
